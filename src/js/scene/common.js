...
(function(){
    ...
    *----------------------------------------------------------------------------*/        
    ...
    phet.scene.canvasAccurateBounds = function( renderToContext, options ) {
        ...
        // xMin
        // TODO: we can still refine y coordinates here, but don't include them in the main minBounds/maxBounds
        while( isFinite( minBounds.xMin ) && isFinite( maxBounds.xMin ) && Math.abs( minBounds.xMin - maxBounds.xMin ) > precision ) {
            ...
            var refinedBounds = scan( idealTransform( new Bounds2( maxBounds.xMin, maxBounds.yMin, minBounds.xMin, maxBounds.yMax ) ) );
        ...
        while( isFinite( minBounds.xMax ) && isFinite( maxBounds.xMax ) && Math.abs( minBounds.xMax - maxBounds.xMax ) > precision ) {
            // use maximum bounds except for the x direction, so we don't miss things that we are looking for
            var refinedBounds = scan( idealTransform( new Bounds2( minBounds.xMax, maxBounds.yMin, maxBounds.xMax, maxBounds.yMax ) ) );
        ...
        // yMin
        // TODO: we can still refine x coordinates here, but don't include them in the main minBounds/maxBounds
        while( isFinite( minBounds.yMin ) && isFinite( maxBounds.yMin ) && Math.abs( minBounds.yMin - maxBounds.yMin ) > precision ) {
            ...
            var refinedBounds = scan( idealTransform( new Bounds2( maxBounds.xMin, maxBounds.yMin, maxBounds.xMax, minBounds.yMin ) ) );
        ...
        while( isFinite( minBounds.yMax ) && isFinite( maxBounds.yMax ) && Math.abs( minBounds.yMax - maxBounds.yMax ) > precision ) {
            // use maximum bounds except for the y direction, so we don't miss things that we are looking for
            var refinedBounds = scan( idealTransform( new Bounds2( maxBounds.xMin, minBounds.yMax, maxBounds.xMax, maxBounds.yMax ) ) );
    ...
    // console.log( phet.scene.canvasAccurateBounds( function( context ) { context.fillRect( 100, 100, 200, 200 ); } ) );
    if( debugChromeBoundsScanning ) {
        console.log( phet.scene.canvasAccurateBounds( function( context ) { context.fillText( 'test string', 0, 0 ); } ) );
    }
})();