...
@compute @workgroup_size(256)fn main(@builtin(global_invocation_id)N:M,@builtin(local_invocation_id)k:M){let p=N.x;let K=gP(p);var agg=hc(K);aG[k.x]=agg;for(var i=e;i<firstTrailingBit(o);i+=f){workgroupBarrier();if k.x+(f<<i)<o{let au=aG[k.x+(f<<i)];agg=eB(agg,au);}workgroupBarrier();aG[k.x]=agg;}if k.x==e{aQ[p>>firstTrailingBit(o)]=agg;}}`
