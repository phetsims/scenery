...
@compute @workgroup_size(256)fn main(@builtin(global_invocation_id)N:M,@builtin(local_invocation_id)k:M){let p=N.x;let K=gH(p);var agg=g3(K);aF[k.x]=agg;for(var i=e;i<firstTrailingBit(o);i+=f){workgroupBarrier();if k.x+(f<<i)<o{let au=aF[k.x+(f<<i)];agg=ey(agg,au);}workgroupBarrier();aF[k.x]=agg;}if k.x==e{aN[p>>firstTrailingBit(o)]=agg;}}`
