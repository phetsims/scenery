...
// WARNING: If you add keys here, you must also update invalidatePeerAccessibleName() (and related tests) so every
// declared fast-path key has concrete peer patch logic.
const ACCESSIBLE_NAME_FAST_PATH_KEYS: readonly ( keyof ParallelDOMOptions )[] = [ 'innerContent', 'ariaLabel', 'labelContent' ];
...
// WARNING: If you add keys here, update the paragraph fast-path patching path so every declared key is handled.
const ACCESSIBLE_PARAGRAPH_FAST_PATH_KEYS: readonly ( keyof ParallelDOMOptions )[] = [ 'accessibleParagraphContent' ];
...
type ParallelDOMSelfOptions = {
  ...
  accessibleVisibleProperty?: TReadOnlyProperty<boolean> | null;
...
export default class ParallelDOM extends PhetioObject {
  ...
  private readonly _accessibleVisibleProperty: TinyForwardingProperty<boolean>;
  ...
  protected disposeParallelDOM(): void {
    ...
    if ( isTReadOnlyProperty( this._accessibleParagraph ) &&
         ...
         // When the accessibleParagraph can be rendered more efficiently, _onPDOMContentChangeListener
         // will not be linked.
  ...
  private onAccessibleNamePropertyChange(): void {
    ...
    ParallelDOM.evaluateBehaviorOptions( this as unknown as Node, this._accessibleNameBehavior, this._accessibleName, ( behaviorOptions, hasOtherNodeCallbacks ) => {
      ...
      if ( requireRender ) {
        ...
        this.invalidatePeerAccessibleName( behaviorOptions );
      }
    } );
  ...
  private invalidatePeerAccessibleName( behaviorOptions: ParallelDOMOptions ): void {
    const innerContent = behaviorOptions.innerContent === undefined ? undefined : unwrapPDOMValueTypeProperty( behaviorOptions.innerContent );
    const labelContent = behaviorOptions.labelContent === undefined ? undefined : unwrapPDOMValueTypeProperty( behaviorOptions.labelContent );
    const ariaLabel = behaviorOptions.ariaLabel === undefined ? undefined : unwrapPDOMValueTypeProperty( behaviorOptions.ariaLabel );
  ...
  public setAccessibleParagraph( accessibleParagraph: PDOMValueType ): void {
    if ( accessibleParagraph !== this._accessibleParagraph ) {
      ...
      ParallelDOM.evaluateBehaviorOptions( this as unknown as Node, this._accessibleParagraphBehavior, accessibleParagraph, ( behaviorOptions, hasOtherNodeCallbacks ) => {
        const requireRender = ParallelDOM.behaviorOptionsRequireRender(
          behaviorOptions,
  ...
  /**
   * Called when our accessibleVisible Property changes values.
   */
  private onAccessibleVisiblePropertyChange( visible: boolean ): void {
    this._pdomDisplaysInfo.onPDOMVisibilityChange( visible );
  }

  /**
   * Sets what Property our accessibleVisibleProperty is backed by, so that changes to this provided Property will change this
   * Node's pdom visibility, and vice versa. This does not change this._accessibleVisibleProperty. See TinyForwardingProperty.setTargetProperty()
   * for more info.
   */
  public setAccessibleVisibleProperty( newTarget: TReadOnlyProperty<boolean> | null ): this {
  ...
  /**
   * See setAccessibleVisibleProperty() for more information
   */
  public set accessibleVisibleProperty( property: TReadOnlyProperty<boolean> | null ) {
    this.setAccessibleVisibleProperty( property );
  }

  /**
   * See getAccessibleVisibleProperty() for more information
   */
  public get accessibleVisibleProperty(): TProperty<boolean> {
    return this.getAccessibleVisibleProperty();
  }


  /**
   * Get this Node's accessibleVisibleProperty. See Node.getVisibleProperty for more information
   */
  public getAccessibleVisibleProperty(): TProperty<boolean> {
   ...
   * order.
   */
  public setAccessibleVisible( visible: boolean ): void {
    this.accessibleVisibleProperty.value = visible;
  }

  public set accessibleVisible( visible: boolean ) { this.setAccessibleVisible( visible ); }

  public get accessibleVisible(): boolean { return this.isAccessibleVisible(); }
  ...
  private addAccessibleResponse( utterance: TAlertable, responseCategory: ResponseCategory = 'other', providedOptions?: DescriptionResponseOptions ): void {
    ...
    if ( alertWhenNotDisplayed ) {
      ...
      globalDescriptionQueue.addAccessibleResponse( alertableToSpeak, responseCategory, flush );
    }
    else {
      ...
      for ( let i = 0; i < connectedDisplays.length; i++ ) {
        ...
        // Only speak if the Node is globally visible with both `visible` and `accessibleVisible`.
        ...
        if ( display.isAccessible() && nodeDisplayed ) {
          display.descriptionUtteranceQueue.addToBack( alertableToSpeak, responseCategory );
        }
      }
    }
  }
   ...
   */
  private static evaluateBehaviorOptions<AllowedKeys extends keyof ParallelDOMOptions>(
    node: Node,
